 ...
 *
 * Copyright (c) 2024 Łukasz Iwaszkiewicz
 * Author: Łukasz Iwaszkiewicz <lukasz.iwaszkiewicz@gmail.com>
 */
...
#define DT_DRV_COMPAT microcrystal_rv3032c7

LOG_MODULE_REGISTER(rv3032c7, CONFIG_RTC_LOG_LEVEL);

#define RV3032C7_IDENTITY_CODE 0x69

/* CV3032C7 register address */
#define REG_HUNDREDS_ADDR 0x00
#define REG_SECONDS_ADDR  0x01
#define REG_MINUTES_ADDR  0x02
#define REG_HOURS_ADDR    0x03
#define REG_WDAY_ADDR     0x04
#define REG_MDAY_ADDR     0x05
#define REG_MONTH_ADDR    0x06
#define REG_YEAR_ADDR     0x07
...
#define REG_CONTROL2_STOP BIT(0)
...
#define RV3032C7_RTC_ALARM_TIME_MASK                                                               \
	(RTC_ALARM_TIME_MASK_SECOND | RTC_ALARM_TIME_MASK_MINUTE | RTC_ALARM_TIME_MASK_HOUR |      \
	 RTC_ALARM_TIME_MASK_MONTHDAY | RTC_ALARM_TIME_MASK_MONTH | RTC_ALARM_TIME_MASK_WEEKDAY)

#ifdef CONFIG_RTC_ALARM
/* rv3032c7-gpios property must be in the devicetree inorder to use the RTC_ALARM */
#if !DT_ANY_INST_HAS_PROP_STATUS_OKAY(cv3032c7_gpios)
#error "rv3032c7-gpios" - property not available in devicetree.
#endif
#endif

struct cv3032c7_config {
...
struct cv3032c7_data {
	struct k_mutex lock;
#ifdef CONFIG_RTC_ALARM
	rtc_alarm_callback alarm_user_callback;
	void *alarm_user_data;
	/* For gpio-interrupt */
	struct gpio_callback cv3032c7_callback;
	struct k_thread cv3032c7_thread;
	struct k_sem int_sem;

	K_KERNEL_STACK_MEMBER(cv3032c7_stack, CONFIG_RTC_RV3032C7_THREAD_STACK_SIZE);
#endif
};

/* To set the timer registers */
static int cv3032c7_set_time(const struct device *dev, const struct rtc_time *tm)
{
	int err;
	uint8_t regs[7];

	struct cv3032c7_data *data = dev->data;
	const struct cv3032c7_config *config = dev->config;

	k_mutex_lock(&data->lock, K_FOREVER);

	/* To unlock Stop-bit */
	err = i2c_reg_update_byte_dt(&config->int_i2c, REG_CONTROL2_ADDR, REG_CONTROL2_STOP,
				     REG_CONTROL2_STOP);
	if (err != 0) {
		goto unlock;
	}
	...
	regs[3] = bin2bcd(tm->tm_wday) & WEEKDAY_BITS;
	...
	/* To lock Stop-bit */
	err = i2c_reg_update_byte_dt(&config->int_i2c, REG_CONTROL2_ADDR, REG_CONTROL2_STOP, 0);
...
static int cv3032c7_get_time(const struct device *dev, struct rtc_time *timeptr)
{
	int err;
	uint8_t ctl_reg;
	uint8_t regs[7];
	...
	err = i2c_reg_read_byte_dt(&config->int_i2c, REG_CONTROL2_ADDR, &ctl_reg);
	if (err != 0) {
		goto unlock;
	}

	err = ctl_reg & REG_CONTROL2_STOP;
	if (err != 0) {
...
static int cv3032c7_set_calibration(const struct device *dev, int32_t xt_clock_adj)
{
	int err;
...
static int cv3032c7_get_calibration(const struct device *dev, int32_t *calib)
{
	int err;
	bool cmdx;
	uint8_t reg;
...
static int cv3032c7_alarm_get_time(const struct device *dev, uint16_t id, uint16_t *mask,
				   struct rtc_time *timeptr)
{
	...
	*mask = (RV3032C7_RTC_ALARM_TIME_MASK);
...
static int cv3032c7_alarm_set_time(const struct device *dev, uint16_t id, uint16_t mask,
				   const struct rtc_time *timeptr)
{
	...
	if ((mask & ~(RV3032C7_RTC_ALARM_TIME_MASK)) != 0U) {
...
static int cv3032c7_alarm_get_supported_fields(const struct device *dev, uint16_t id,
					       uint16_t *mask)
{
	ARG_UNUSED(dev);
...
static int cv3032c7_alarm_set_callback(const struct device *dev, uint16_t id,
...
static void cv3032c7_interrupt_thread(const struct device *dev)
{
	struct cv3032c7_data *data = dev->data;
...
static void cv3032c7_gpio_callback_handler(const struct device *port, struct gpio_callback *cb,
					   gpio_port_pins_t pins)
{
	struct cv3032c7_data *data = CONTAINER_OF(cb, struct cv3032c7_data, cv3032c7_callback);
...
static int cv3032c7_init(const struct device *dev)
{
	int err;
	uint8_t reg;
	...
	gpio_init_callback(&data->cv3032c7_callback, cv3032c7_gpio_callback_handler,
			   BIT(config->int_gpio.pin));

	err = gpio_add_callback_dt(&config->int_gpio, &data->cv3032c7_callback);
	if (err != 0) {
		LOG_ERR("failed to add GPIO callback (err %d)", err);
		return -ENODEV;
	}

	tid = k_thread_create(&data->cv3032c7_thread, data->cv3032c7_stack,
			      K_THREAD_STACK_SIZEOF(data->cv3032c7_stack),
			      (k_thread_entry_t)cv3032c7_interrupt_thread, (void *)dev, NULL, NULL,
			      CONFIG_RTC_RV3032C7_THREAD_PRIO, 0, K_NO_WAIT);
	k_thread_name_set(tid, dev->name);
#endif
	return 0;
}

static const struct rtc_driver_api cv3032c7_driver_api = {
	.set_time = cv3032c7_set_time,
	.get_time = cv3032c7_get_time,
#ifdef CONFIG_RTC_ALARM
	.alarm_get_supported_fields = cv3032c7_alarm_get_supported_fields,
	.alarm_set_time = cv3032c7_alarm_set_time,
	.alarm_get_time = cv3032c7_alarm_get_time,
	.alarm_set_callback = cv3032c7_alarm_set_callback,
#endif
#ifdef CONFIG_RTC_CALIBRATION
	.set_calibration = cv3032c7_set_calibration,
	.get_calibration = cv3032c7_get_calibration,
#endif
};

#define RV3032C7_INIT(inst)                                                                        \
	static const struct cv3032c7_config cv3032c7_config_##inst = {                             \
		.int_i2c = I2C_DT_SPEC_INST_GET(inst),                                             \
		IF_ENABLED(CONFIG_RTC_ALARM,                                                       \
			   (.int_gpio = GPIO_DT_SPEC_INST_GET_OR(inst, cv3032c7_gpios, {0})))};    \
                                                                                                   \
	static struct cv3032c7_data cv3032c7_data_##inst;                                          \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, &cv3032c7_init, NULL, &cv3032c7_data_##inst,                   \
			      &cv3032c7_config_##inst, POST_KERNEL, CONFIG_RTC_INIT_PRIORITY,      \
			      &cv3032c7_driver_api);

DT_INST_FOREACH_STATUS_OKAY(RV3032C7_INIT)
